...
(function(window, document){
  ...
  function setState(el, state, on){
    if(on) el.classList.add(state); else el.classList.remove(state);
  }
  ...
  function attachToElement(el){
    ...
    function setPlayingFlag(on){ setState(el, 'playing', on); }
    function setErrorFlag(on){ setState(el, 'error', on); }
    ...
    fetchText(url).then(function(text){
      ...
      setErrorFlag(true);
      setTimeout(function(){ setErrorFlag(false); }, 1000);
    });
    ...
    function play(){
      try{
        if(!loaded || !audioSrc){
          ...
          setErrorFlag(true);
          setTimeout(function(){ setErrorFlag(false); }, 700);
          return;
        }
        ...
        if(player !== audioEl){
          player.addEventListener('ended', function(){ setPlayingFlag(false); });
          player.addEventListener('error', function(){ setPlayingFlag(false); setErrorFlag(true); setTimeout(function(){ setErrorFlag(false); }, 1000); });
        } else {
          // audioEl will fire events itself
          audioEl.onended = function(){ setPlayingFlag(false); };
          audioEl.onerror = function(){ setPlayingFlag(false); setErrorFlag(true); setTimeout(function(){ setErrorFlag(false); }, 1000); };
        }
    ...
    el.__audioBase64 = {
      play: play,
      unload: function(){ cleanupObjectUrl(); try{ if(audioEl && audioEl.parentNode) audioEl.parentNode.removeChild(audioEl); }catch(e){} }
    };
  }
